...
-(void) getrecord{
  ...
  labelcount.text= [strcount4 stringByAppendingString:strcount3];
    ...
    NSString *zsql=[NSString stringWithFormat:@"select * from tk where rowid=%d",z];
    const char *sql= [zsql cStringUsingEncoding:NSASCIIStringEncoding];
    sqlite3_stmt *stmt;
   ...
   // _zlabel.text=[NSString stringWithUTF8String:pname];
    ...
    const char* zstyle= (const char*)sqlite3_column_text(stmt, 10);//读取stmt中存储的第2列数据
    NSString *zstyle1=@"(";
    NSString *zstyle2=@")";
  ...
  NSString  *zstr3 = [zstr2 stringByAppendingString:zstyle2];
   NSString *zstr4=[zstr3 stringByAppendingString:zstr1];
    
    NSMutableAttributedString *zstr5 = [[NSMutableAttributedString alloc] initWithString:zstr4];
    
    [zstr5 addAttribute:NSForegroundColorAttributeName value:[UIColor blueColor] range:NSMakeRange(0,7)];
   // zlabel.text= [zstr1 stringByAppendingString:zstr4];
   zlabel.attributedText=zstr5;
    zlabel.hidden=false;
   
    if (sqlite3_column_text(stmt, 2)!=NULL)
    {
        const char* zlbla = (const char*)sqlite3_column_text(stmt, 2);//读取stmt中存储的第2列数据
        NSString *str1=@"A、";
    ...
    {
        const char* zlblb = (const char*)sqlite3_column_text(stmt, 3);//读取stmt中存储的第2列数据
        NSString *str1=@"B、";
    ...
    {
        const char* zlblc = (const char*)sqlite3_column_text(stmt, 4);//读取stmt中存储的第2列数据
        NSString *str1=@"C、";
    ...
    {
        const char* zlbld = (const char*)sqlite3_column_text(stmt, 5);//读取stmt中存储的第2列数据
        NSString *str1=@"D、";
    ...
    {
        const char* zlble = (const char*)sqlite3_column_text(stmt, 6);//读取stmt中存储的第2列数据
        NSString *str1=@"E、";
    ...
    {
        const char* zlblf = (const char*)sqlite3_column_text(stmt, 7);//读取stmt中存储的第2列数据
        NSString *str1=@"F、";
    ...
    {
        const char* zlblok = (const char*)sqlite3_column_text(stmt, 10);//读取stmt中存储的第2列数据
       // NSString *str1=@"";
        NSString *str = [NSString stringWithUTF8String:zlblok];
